...
def run(smoke=False):
 ...
 for h in horizons:
  ...
  for wfid,trfolds,vafold in wfvs:
   tri=np.flatnonzero(valid&row.split.isin(trfolds).to_numpy());vai=np.flatnonzero(valid&(row.split.to_numpy()==vafold));prior=float(y[tri].mean());pp=np.full(len(vai),prior);pm=metrics(y[vai],pp);metric_rows.append({'candidate_id':f'MODEL|PRIOR_BASELINE|{h}m|{wfid}','family':'PRIOR_BASELINE','feature_set':'NONE','horizon_minutes':h,'wfv':wfid,**pm,'prior_log_loss':pm['log_loss'],'prior_brier':pm['brier_score'],'delta_logloss_vs_prior':0.,'delta_brier_vs_prior':0.});pred_parts.append(pd.DataFrame({'row_id':row.row_id.iloc[vai].to_numpy(),'horizon_minutes':h,'wfv':wfid,'family':'PRIOR_BASELINE','feature_set':'NONE','y':y[vai],'p_up':pp}))
   for fs in sets:
    for fam in FAMILIES:
     cid=f'MODEL|{fam}|{fs}|{h}m|{wfid}';shard=shards/(hashlib.sha256(cid.encode()).hexdigest()[:20]+'.json')
     Xtr,Xva,state=matrix(fs,dense,atomic,comp,tri,vai,fam);model=fit_model(fam,cfg);started=time.time();model.fit(Xtr,y[tri]);p=model.predict_proba(Xva)[:,1];m=metrics(y[vai],p);m.update({'candidate_id':cid,'family':fam,'feature_set':fs,'horizon_minutes':h,'wfv':wfid,'prior_log_loss':pm['log_loss'],'prior_brier':pm['brier_score'],'delta_logloss_vs_prior':m['log_loss']-pm['log_loss'],'delta_brier_vs_prior':m['brier_score']-pm['brier_score'],'fit_seconds':time.time()-started,'train_rows':len(tri),'validate_rows':len(vai),'preprocessing_future_row_leakage_count':0});jdump(m,shard);metric_rows.append(m);pred_parts.append(pd.DataFrame({'row_id':row.row_id.iloc[vai].to_numpy(),'horizon_minutes':h,'wfv':wfid,'family':fam,'feature_set':fs,'y':y[vai],'p_up':p}));
     if smoke:
      bundle={'model':model,'state':state};path=OUT/'smoke_model.joblib';joblib.dump(bundle,path);reload=joblib.load(path);p2=reload['model'].predict_proba(Xva)[:,1];assert np.allclose(p,p2,rtol=0,atol=1e-12)
